...
## 🔴 ОБЯЗАТЕЛЬНЫЙ ПРОТОКОЛ ВИЗУАЛЬНОЙ ПРОВЕРКИ (после каждого раунда)

> **Раунд не считается завершённым, пока не пройдена полная визуальная проверка.**
> Написать код — только половина работы. Вторая половина — убедиться глазами.

### Шаг 1 — CSS Guard (до деплоя)

```bash
make css-guard
```
❌ FAIL → деплой заблокирован, исправить до продолжения.
...
### Шаг 3 — Визуальная проверка Chrome DevTools MCP (ОБЯЗАТЕЛЬНО)

После каждого деплоя выполнить последовательно:

```
1. navigate_page(url='http://localhost:8002')
   wait_for(text=['Мои переводы'])

2. take_screenshot()                        ← главный экран
   Проверить: layout не сломан, нет overflow, шрифты читаемы

3. list_console_messages(types=['error','warn'])
   Ноль JS-ошибок? Ноль 404? Нет SW-проблем?

4. Тёмная тема:
   click(кнопка переключения темы)
   take_screenshot()                        ← все элементы читаемы?

5. Светлая тема:
   click(кнопка переключения темы обратно)
   take_screenshot()                        ← нет белых "дыр" в тёмном UI?

6. Мобильный viewport 375px:
   emulate(viewport='375x812x2,mobile,touch')
   take_screenshot()                        ← нет горизонтального скролла?
   Кнопки ≥44px? Поиск в одну строку?

7. ЕСЛИ в раунде менялись модалки — обязательно:
   click(кнопка открытия модалки)
   take_screenshot()
   evaluate_script(() => {
     const s = getComputedStyle(document.querySelector('.modal-overlay'));
     return { bg: s.background, pos: s.position, z: s.zIndex };
   })
   → background НЕ прозрачный? position=fixed? z-index>=1000?

8. Вернуть viewport:
   emulate(viewport='1280x800')
```

### Шаг 4 — Playwright Visual Smoke (если изменялся CSS layout)

```bash
make visual-check
# Скриншоты → .agents/designer/screenshots/YYYY-MM-DDTHH-MM/
```

### Шаг 5 — Запись в design-log.md (ОБЯЗАТЕЛЬНО)

Дополнить записью о результатах раунда:
- Что изменено
- Скриншоты (inline или путь к файлу)
- Найденные баги / замечания
- Оценка: ✅ OK / ⚠️ Замечания / ❌ Блокеры

### ✅ Чеклист конца раунда (все пункты обязательны):

```
[ ] make css-guard — OK
[ ] make deploy — OK
[ ] Скриншот Dashboard тёмная тема — проверен визуально
[ ] Скриншот Dashboard светлая тема — проверен визуально
[ ] Скриншот мобильный 375px — нет горизонтального скролла
[ ] console errors/warns — ноль или задокументированы
[ ] Если изменены модалки: overlay проверен программно (evaluate_script)
[ ] design-log.md обновлён с результатами раунда
```
