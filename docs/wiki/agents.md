# Агенты Разработки

В проекте используются легковесные роли ревью. Сейчас они ведутся как
письменные роли в wiki, позже их можно сопоставить с реальными автоматическими
агентами.

Английские названия в скобках являются стабильными идентификаторами будущих
агентских сервисов, а вся рабочая документация ведется на русском языке.

## Роли

### Владелец Продукта (`Product Owner Agent`)

Отвечает за пользовательскую ценность, понятность рабочего процесса и контроль
объема работ.

### Аналитик (`Analyst Agent`)

Отвечает за требования, пограничные случаи и критерии приемки.

### Архитектор (`Architect Agent`)

Отвечает за границы модулей, контракты артефактов и долгосрочную
поддерживаемость.

### Агент Перевода (`Translation Agent`)

Отвечает за качество перевода, стиль, языковую обработку и поведение глоссария.

### Агент Таймингов (`Timing Agent`)

Отвечает за длительность сегментов, плотность речи, подгонку таймингов и
предотвращение наложений.

### Агент Проверки Аудио (`Audio QA Agent`)

Отвечает за громкость, тишину, клиппинг, сведение дорожек и базовую проверку
итогового медиа.

### Агент Лингвистической Проверки (`Linguistic QA Agent`)

Отвечает за смысловую точность, имена, числа и качество целевого языка.

### Инженер По Тестированию (`Test Engineer Agent`)

Отвечает за сфокусированное покрытие тестами и повторяемую верификацию.

### Менеджер Релизов (`Release Manager Agent`)

Отвечает за версионирование, готовность журнала изменений и чистоту git-flow.

## Комментарии По Этапам

### Этап: Архитектурный Каркас Ядра

#### Владелец Продукта

- Архитектурный каркас соблюдает запрошенный порядок: сначала ядро, затем CLI,
  затем UI.
- Корневой README пришлось обновить, потому что старая версия описывала продукт
  только как скрипт перевода с английского на русский.
- Следующий этап CLI должен вывести исходный язык, целевой язык, режим перевода,
  стиль, стратегию голосов, проверку качества и рабочую директорию.
- Для следующего этапа нужен конкретный контракт `qa_report.json`, даже если
  первые проверки будут минимальными.

#### Аналитик

- Перевод между любыми языками представлен в конфигурации, но возможности
  провайдеров и правила резервного поведения пока не реализованы.
- Режимы, стили и стратегии голосов присутствуют как значения конфигурации.
  Следующие этапы должны привязать их к реальному рендеру и подбору голосов.
- n8n исключен из первой версии исполнения, но подготовка webhook есть в
  документации и версионированных событиях.

#### Архитектор

- Граница пакета ядра чистая и не зависит от CLI, UI и медиа-провайдеров.
- Устаревший `main.py` все еще владеет исполняемым процессом и должен быть
  разобран до того, как CLI станет основной точкой входа.
- Идентификатор проекта использует единое значение для `project.id` и
  `work_dir.name`.
- Метаданные артефактов теперь имеют типизированные записи, сохраняя простую
  карту поиска для совместимости.
- База этапов и задач есть, но перед API/UI нужен настоящий раннер этапов.

#### Инженер По Тестированию

- Сфокусированные тесты покрывают цикл сохранения и восстановления
  конфигурации, тайминги сегментов, сохранение проекта, записи артефактов и
  webhook-события.
- Текущая команда проверки:
  `PYTHONPATH=src python3 -m unittest discover -s tests`.
- Команда проверки синтаксиса:
  `python3 -m compileall -q src tests`.
- Используем стандартный тестовый раннер до осознанного добавления менеджера
  dev-зависимостей.

#### Менеджер Релизов

- Версия остается `0.1.0`, пока устанавливается модель ядра.
- Ветка следовала документированному процессу от `develop`.
- Требование Python согласовано как `3.11+`.
- Перед слиянием в `develop` нужно повторить тесты, проверку синтаксиса и
  `git diff --check`.

### Этап: Сервисы Пайплайна Ядра

#### Владелец Продукта

- Этап развивает запрошенный порядок: сервисы пайплайна ядра создаются до
  CLI и UI.
- Режим, стиль и стратегия голосов пока остаются в основном конфигурацией.
  Следующие этапы должны доказать поведение для `voiceover`, `subtitles` и одного из
  `dub` или `dual_audio`.
- Автономный путь стал ближе: этапы с имитационными провайдерами могут
  выполняться без ручной проверки. Проверка качества и политики повторов
  остаются будущей работой.

#### Аналитик

- Контракты провайдеров получают `PipelineConfig`, поэтому настройки языков и
  стиля смогут дойти до реальных провайдеров.
- Текущие этапы покрывают извлечение, распознавание, перевод, TTS и рендер. В
  плане остаются анализ медиа, анализ спикеров, подбор голосов, подгонка
  таймингов, сведение, QA и экспорт.
- Готовность webhook пока на уровне контрактов; генерация событий в раннере и
  этапах ожидается в будущем этапе API задач.

#### Архитектор

- Сервисы пайплайна провайдер-независимы и тестируются имитационными
  провайдерами.
- `PipelineRunner` обновляет `VideoProject.status` в `running`, `completed` или
  `failed`.
- `StageRun` содержит отметки времени, входы, выходы и ошибки.
- Последующие этапы проверяют обязательные артефакты до выполнения.
- Результат TTS регистрируется как типизированный артефакт для проверки
  возобновления и экспорта.

#### Инженер По Тестированию

- Тесты ядра перенесены в `tests/unit`, поэтому модульная проверка покрывает
  схемы, хранилище, webhook, раннер и проверки версии.
- Интеграционные тесты покрывают успешный путь имитационных провайдеров и
  предварительные условия отказов.
- E2E и нагрузочные проверки исполняемы и содержат дымовые тесты проверок.
- Текущая полная проверка:
  `PYTHONPATH=src python3 -m unittest discover -s tests`.

#### Менеджер Релизов

- Метаданные версии согласованы на `0.2.0` в `VERSION`, `pyproject.toml` и
  `__version__` пакета.
- `change.log` содержит запись `TVIDEO-002-core-pipeline-services`.
- Имя ветки соответствует согласованному формату `TVIDEO-XXX-short-name`.

### Этап: Основа CLI

#### Владелец Продукта

- CLI закрывает базовый пользовательский путь: создать проект, запустить
  пайплайн, посмотреть статус, артефакты и настройки.
- Реальные провайдеры намеренно не смешаны с этим этапом, чтобы CLI можно было
  проверить без внешних моделей и медиа-зависимостей.
- Следующий этап должен добавить выбор реального набора провайдеров и сохранить
  тот же пользовательский контракт команд.

#### Аналитик

- Все ключевые параметры перевода доступны как CLI-аргументы: исходный язык,
  целевой язык, режим, стиль, адаптация, стратегия голосов, проверка качества,
  глоссарий и список терминов без перевода.
- `run` поддерживает новый проект по входному видео и повторный запуск по
  `--work-dir`, что готовит основу для возобновления задач.
- Команды возвращают JSON, поэтому их можно будет использовать из UI, скриптов
  и будущей API-обвязки.

#### Архитектор

- CLI является тонким адаптером и вызывает `ProjectStore`, `PipelineRunner` и
  этапы ядра, не дублируя бизнес-логику пайплайна.
- Имитационные провайдеры находятся в CLI только как временный проверочный
  набор. Реальные адаптеры должны жить в отдельных модулях провайдеров.
- Точка входа `translate-video` добавлена через `pyproject.toml`.

#### Инженер По Тестированию

- Модульные тесты покрывают `init` и чтение сохраненной конфигурации.
- Интеграционный тест покрывает `run`, `status` и `artifacts` на полном
  имитационном пайплайне.
- Перед слиянием в `develop` нужно повторить полный discovery, отдельные unit и
  integration проверки, `compileall` и `git diff --check`.

#### Менеджер Релизов

- Версия поднята до `0.3.0` как совместимое расширение функциональности.
- `change.log` содержит запись `TVIDEO-003-cli-foundation`.
- Ветка создана от актуального `develop` после слияния `TVIDEO-002`.

### Этап: Адаптеры Устаревшего Скрипта

#### Владелец Продукта

- CLI теперь может запускаться не только на имитациях, но и через реальные
  адаптеры устаревшего скрипта с `--provider legacy`.
- Пользовательский контракт команд не изменился, поэтому будущий UI сможет
  использовать тот же путь запуска.
- Адаптеры пока сохраняют поведение закадровой озвучки; полноценные режимы
  дубляжа, субтитров и нескольких дорожек остаются отдельными этапами.

#### Аналитик

- Адаптер перевода использует `source_language` и `target_language` из
  конфигурации, а не жестко заданную пару языков.
- TTS получил базовую карту голосов по языкам, но подбор голосов по спикерам
  еще не реализован.
- Рендерер реализует voiceover-сценарий и использует настройку громкости
  оригинального аудио из `PipelineConfig`.

#### Архитектор

- Зависимости MoviePy, faster-whisper, deep-translator и edge-tts импортируются
  лениво внутри адаптеров, поэтому модульные тесты ядра не зависят от тяжелых
  пакетов.
- Адаптеры живут в пакетах провайдеров и подключаются к CLI через фабрику
  этапов, не меняя контракты пайплайна.
- `main.py` пока оставлен как устаревшая совместимая точка входа; следующий
  рефакторинг может сделать его тонкой оберткой над CLI.

#### Инженер По Тестированию

- Unit-тесты покрывают каждый адаптер через fake-фабрики без сетевых вызовов и
  без реальной обработки видео.
- Существующие CLI и pipeline тесты продолжают проверять имитационный путь,
  чтобы быстрый gate оставался стабильным.
- Перед слиянием в `develop` нужно повторить полный discovery, отдельные unit и
  integration проверки, `compileall` и `git diff --check`.

#### Менеджер Релизов

- Версия поднята до `0.4.0` как совместимое добавление реальных провайдеров.
- `change.log` содержит запись `TVIDEO-004-legacy-adapters`.
- Изменения находятся в ветке `TVIDEO-004-legacy-adapters`, созданной от
  актуального `develop`.

### Этап: Предварительная Проверка Реального Видео

#### Владелец Продукта

- Добавлена безопасная команда `preflight`, которая позволяет проверить
  реальный файл и окружение без запуска дорогостоящего перевода.
- Локальное видео пользователя рассматривается как ручной тестовый вход, но не
  добавляется в репозиторий.
- Следующий шаг после зеленого preflight: подготовить окружение и запускать
  короткий контролируемый фрагмент, а не сразу полный длинный перевод.

#### Аналитик

- Отчет показывает общий `ok` и список атомарных проверок: входной файл,
  Python-модули legacy-провайдера и системные команды FFmpeg/FFprobe.
- Команда поддерживает `--provider fake` и `--provider legacy`, поэтому ее
  можно использовать и для быстрых проверок CLI, и для реального окружения.
- Формат отчета JSON-совместим и пригоден для будущего UI/API.

#### Архитектор

- Логика проверки вынесена в `translate_video.core.preflight`, а CLI только
  сериализует отчет.
- Проверки зависимостей используют ленивое обнаружение модулей и не импортируют
  тяжелые runtime-пакеты.
- Команда не создает проект и не меняет состояние пайплайна.

#### Инженер По Тестированию

- Unit-тесты покрывают успешный fake-preflight, отсутствующий файл, отсутствие
  legacy-зависимостей, успешный legacy-preflight через инъекцию проверяющих
  функций и CLI JSON-отчет.
- Реальные видео остаются ручными или opt-in тестами, чтобы основной gate был
  быстрым и воспроизводимым.
- Перед слиянием в `develop` нужно повторить полный discovery, отдельные unit и
  integration проверки, `compileall` и `git diff --check`.

#### Менеджер Релизов

- Версия поднята до `0.5.0` как совместимое добавление диагностической команды.
- `change.log` содержит запись `TVIDEO-005-real-video-preflight`.
- Ветка создана от актуального `develop` и не содержит медиафайлов.
