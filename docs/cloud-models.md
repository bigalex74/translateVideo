...
## Динамическая Загрузка Моделей И Баланса

UI не хранит полный список профессиональных моделей в коде. При выборе
агрегатора он обращается к backend API:

```text
GET /api/v1/providers/{provider}/models
GET /api/v1/providers/{provider}/balance
```

Backend читает ключи из `.env` и сам вызывает OpenAI-compatible `/models`.
Ключи не отправляются в браузер.

Для NeuroAPI используется:

- модели: `GET https://neuroapi.host/v1/models`;
- расход: `GET https://neuroapi.host/v1/dashboard/billing/usage`.

Публичная документация NeuroAPI описывает `total_usage` в центах USD. Endpoint
остатка баланса в документации не указан, поэтому UI показывает расход и
сообщение, что остаток недоступен.
