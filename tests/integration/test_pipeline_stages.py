...
class PipelineStagesIntegrationTest(unittest.TestCase):
    ...
    def test_pipeline_records_artifacts_and_stage_runs(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            translate_run = next(run for run in restored.stage_runs if run.stage == Stage.TRANSLATE)
            self.assertEqual(translate_run.progress_current, 1)
            self.assertEqual(translate_run.progress_total, 1)
            self.assertEqual(translate_run.progress_message, "Перевод готов")
